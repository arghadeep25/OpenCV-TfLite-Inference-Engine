...
int main() {
  ...
  if (pose_estimation.load_model(pose_model_path) ==
      tflite::inference::InferenceStatus::OK) {
  ...
  tflite::logging::Logger::getInstance().setOutput(log_file);

  //  std::string image_path = std::string(PROJECT_SOURCE_DIR) +
  //  "/data/test.jpg"; cv::Mat img = cv::imread(image_path);
  //  //   cv::Mat img = cv::Mat::zeros(300, 300, CV_8UC3);
  ...
  //  TfLiteIntArray* dims =
  //  interpreter->tensor(interpreter->inputs()[0])->dims; int inputHeight =
  //  dims->data[1]; int inputWidth = dims->data[2]; int inputChannels =
  //  dims->data[3]; std::cout << "Interpreter expectations" << std::endl;
  ...
  //  // std::cout << "Number of detections: " << num_detections->data <<
  //  std::endl;
  //
  //  // interpreter->SeAllow
  //
